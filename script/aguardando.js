...
async function verificarSorteioConcluido() {
    try {
      const response = await fetch("http://127.0.0.1:3000/sorteio/status"); // substitua pela rota real
      if (!response.ok) throw new Error("Erro ao consultar o status do sorteio");
  
      const resultado = await response.json();
  
      // Aqui você decide com base em alguma propriedade (exemplo: resultado.finalizado === true)
      if (resultado.finalizado) {
        // Redireciona para a página de resultado
        window.location.href = "resultado.html"; // substitua pelo caminho real
      }
    } catch (error) {
